...
import { getStudyById, getSurveyById, getParticipants, enrollParticipant } from '../../lib/api';
...
export default function StudyDetailPage() {
  ...
  async function loadStudy() {
    try {
      ...
      // Fetch full survey details (includes questions) for each survey
      const surveyDetails = await Promise.all(
        studyData.surveys.map(s => getSurveyById(s.id))
      );
  ...
  return (
    <div className="min-h-screen bg-rose-950 text-rose-50 font-sans">
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
        ...
        {/* Surveys Section */}
        <section>
          <h2 className="text-lg font-semibold text-white mb-4">Surveys</h2>
          {surveys.length === 0 ? (
                    ...
                    <ul className="space-y-2">
                      {survey.questions.map((q, idx) => (
                        <li key={q.id} className="flex items-start gap-3 text-sm">
                          <span className="mt-0.5 w-5 h-5 rounded-full bg-rose-600/20 border border-rose-600/30 flex items-center justify-center text-xs text-rose-400 shrink-0">
                            {idx + 1}
                          </span>
                          <div>
        ...
        <section className="rounded-2xl border border-rose-800/40 bg-white/5 p-6">
          ...
          {enrollError && (
            <p className="mt-3 text-sm text-red-300">{enrollError}</p>
          )}
          {enrollSuccess && (
            <p className="mt-3 text-sm text-green-300">{enrollSuccess}</p>
          )}
        </section>
